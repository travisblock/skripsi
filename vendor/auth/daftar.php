	...
	if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM vendor WHERE username='$username'")) < 1){
		...
		if ($ktp_ext === 'png' || $ktp_ext === 'jpg' || $ktp_ext === 'jpeg') {
			...
			if (move_uploaded_file($ktp_tmp, $ktp_path . '/' . $ktp_new)) {
				$register = mysqli_query($conn, "INSERT INTO vendor(nama_vendor, nama_pemilik, ktp, telp, tgl_daftar, alamat, username, password, status) VALUES('$nama_vendor', '$pemilik_vendor', '$ktp_new', '$no_telp', '$tgl', '$alamat_vendor', '$username', '$password', 0) ");
		...
		echo "<script>alert('Vendor sudah terdaftar');
		window.location.href='../register.php';
		</script>";
	}
